    ...
    private function createView(): string
    {
        return "
            ...
            with get_open_move as (
                ...
                join accounts on (accounts.id = account_id)
                where movements.user_id = user_id() and badge_id = currency() and deleted_at is null and category_id <> category_id()
                ...
                join accounts on (accounts.id = account_id)
                where movements.user_id = user_id() and badge_id = currency() and deleted_at is null and trm <> 1
                and date(date_purchase) >= init_date() and date(date_purchase) <= end_date()
            ), get_balance as (
