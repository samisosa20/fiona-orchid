    ...
    private function createView(): string
    {
        return "
            ...
            with get_amount_month as (
                ...
                SELECT code , if(year(a.created_at) = year(now()), a.init_amount, 0) as init_amount, (select ifnull(sum(amount), 0) from movements where account_id = a.id and year(date_purchase) = year(now())) as balance from accounts a
                join currencies b on (a.badge_id = b.id)
                where a.user_id = user_id()
            ), sum_amount_year as (
